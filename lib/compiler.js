...
function compileJSModule(code, pathBase) {
	var modules = {}
	code = _replaceRequireStatements(code, modules, pathBase)
	return code
}
...
var _replaceRequireStatements = function(code, modules, pathBase) {
	...
	for (var i=0, requireStatement; requireStatement = requireStatements[i]; i++) {
		...
		var rawModulePath = requireStatement.match(_pathnameGroupingRegex)[1],
			...
			modulePath = _findTruePath(searchPath, modules)
		...
		if (modules[modulePath]) {
			...
			code = code.replace(requireStatement, [
					...
					'	// start '+modulePath+' code',
						_replaceRequireStatements(_readFile(modulePath + '.js'), modules, newPathBase),
					'	// end '+modulePath+' module code',
